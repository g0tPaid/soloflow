...
export function QuotePrintPageContent({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const {
    ...
    queryKey: ['quote-print', id, organizationId],
  ...
  useEffect(() => {
    ...
    const urls = [
      ...
      ...(quote.items ?? []).flatMap((item) => [item.imageUrl, item.product?.imageUrl]),
    ]
  ...
  if (sessionStatus === 'loading' || (!isReady && !orgFromUrl)) {
    return <p className="p-8 text-center text-sm text-gray-500">Preparing quote…</p>;
  }

  if (sessionStatus === 'unauthenticated') {
    return (
      <div className="p-8 text-center text-sm text-gray-600">
        <p className="font-medium text-gray-900">Sign in to download this quote</p>
        <p className="mt-2">
          <Link
  ...
  if (isError) {
    return (
      <div className="p-8 text-center text-sm text-red-600">
        <p className="font-medium">Could not load quote</p>
        <p className="mt-2">{error instanceof Error ? error.message : 'Unknown error'}</p>
      </div>
    );
  }

  if (!canFetch || isLoading || isFetching || !quote) {
    return <p className="p-8 text-center text-sm text-gray-500">Preparing quote…</p>;
  }

  const org = orgDetail ?? organization;
  const invoice = quoteAsInvoiceForPrint(quote);
  const safeFilename = `${quote.number.replace(/[^a-zA-Z0-9-_]/g, '_')}.pdf`;
    ...
    <>
      {!embed && (
        <PrintPageToolbar
          ...
          emailSubject={`Quote ${quote.number}`}
        />
      )}
